...
namespace cs225 {
  ...
  double PNG::dist(PNG* other) {
    double out = 0;
    //assert(width_ == other->width() && height_ == other->height()); 
      //std::cout<<other<<std::endl;
      for(unsigned i = 0; i < std::max(width_, other->width()); i++){
        for(unsigned j = 0; j < std::max(height_, other->height()); j++){
          if(i >= width_ || j >= height_) {
             if(i >= other->width() || j >= other->height())
              break;
            out+=(other->getPixel(i,j).val());
            continue;
          } else if(i >= other->width() || j >= other->height()) {
            out+=(getPixel(i,j).val());
            continue;
          }
          out += std::abs(getPixel(i,j).dist(other->getPixel(i,j)));
        }
      }
    return out;
  }
